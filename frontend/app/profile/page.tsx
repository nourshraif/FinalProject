...
function ProfileContent() {
  ...
  const cvFilename = profile?.cv_filename as string | undefined;
  const {
    previewUrl: cvPreviewUrl,
    loading: cvPreviewLoading,
    error: cvPreviewError,
  } = useAuthenticatedCvPreview(
    Boolean(token && cvFilename),
    cvFilename ? `${BASE_URL}/api/profile/cv` : undefined,
    token ?? undefined
  );
    ...
    <div className="min-h-screen px-4 pb-12 pt-24 sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="glass-card mb-4 p-6">
          ...
          {profile?.cv_filename ? (
            <CvPreviewPanel
              filename={profile.cv_filename as string}
              previewUrl={cvPreviewUrl}
              loading={cvPreviewLoading}
              error={cvPreviewError}
              cvText={(profile.cv_text as string) || null}
              title="Your CV"
            />
          ) : (
            <div
